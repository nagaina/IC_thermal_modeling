...
{
	connect(m_ribbonBar, SIGNAL(calculate()), m_canvas, SLOT(calculate()));
	connect(m_ribbonBar, SIGNAL(clear()), m_canvas, SLOT(clear()));
	connect(m_ribbonBar, SIGNAL(openFile(const QString&)), m_canvas, SLOT(loadFile(const QString&)));
}
